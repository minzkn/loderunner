...
- 반응형 스케일링 + 모바일 터치 컨트롤
- 현재 런타임은 `apple2_strict` 프로파일로 고정
- 데모/에디터/자동 교착 감지는 코드에 남아 있지만 고정 프로파일에서는 비활성

## 조작

| 동작 | 키 |
|------|-----|
| 이동 (좌/우) | 방향키 / J,L (`modern` 해제 시 A,D도 지원) |
| 사다리 (상/하) | 방향키 위아래 / I,K (`modern` 해제 시 W,S도 지원) |
| 왼쪽 파기 | Z / U / F1 |
| 오른쪽 파기 | X / O / F2 |
| 시작/재개/일시정지 | ENTER |
| 일시정지 토글 | ESC |
...
| 사운드 토글 | M |
| 게임 모드 전환 (타이틀) | C |
| 규칙 프로파일 전환 (타이틀) | V (현재 빌드에서는 고정 안내만 표시) |
| 레벨 에디터 토글 | E (현재 빌드에서는 비활성) |
| 디버그 레벨 이동 | Shift+N / Shift+P |

에디터 모드(`modern` 프로파일 잠금 해제 시):
- `1~8`: 타일 선택
- `P`: 플레이어 배치
- `E`: 적 배치
- `S`: 저장
...
규칙 프로파일:
- `apple2_strict` (현재 고정): 목숨 5, 데모/에디터/자동 교착 비활성, 레거시 입력
- `c64_strict`: 목숨 5, 레거시 입력 + `Ctrl+A` 중단
- `modern`: 목숨 99, 자동 교착 감지, 데모/에디터 활성, 동적 탈출 사다리
