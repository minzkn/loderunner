...
- `BRICK`: 보라색(Purple/Violet) 벽돌 패턴 (Apple II 원작)
- `SOLID`: 파란 계열 고체 블록
- `LADDER`: 초록/시안 사다리 (Apple II 원작)
- `ESCAPE_LADDER`: 활성 시 초록/시안 점멸 강조
- `BAR`: 주황색 수평 바 (Apple II 원작)
- `TRAP`: 일반 바닥처럼 보이는 함정 시각
- `GOLD`: 노랑 중심, 하이라이트 포함

### 6.3 엔티티 렌더링 규칙

- 플레이어: 흰색(White), 이동 프레임 애니메이션 (Apple II 원작)
- 적: 빨강 계열, 갇힘 상태 별도 연출
